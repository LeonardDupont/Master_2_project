function [neuropile] = subR_fluorescence(neuropile,inputfile,varargin)
...
for roi = 1:N
    ...
    if neuropile.bkg
        ...
        background = (np_mask_seg == (nseg + 2));
        background = uint16(background);
        bkg_trace = zeros(1,t);
        for frame = 1:t
            b = tiffdata(:,:,frame) .* background;
            bkg_trace(frame) = sum(b(:)) / length(find(b~=0)); 
        end
        for roi = 1:N
            neuropile.intensity{nseg+2,roi} = bkg_trace;
        end
    end
